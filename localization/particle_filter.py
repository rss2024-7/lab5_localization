...
class ParticleFilter(Node):
    ...
    def laser_callback(self, msg):
        ...
        # keep = int(self.num_particles * 0.975)

        # prevent error
        if np.count_nonzero(weights) < self.num_particles: return
    ...
    def pose_callback(self, msg):
        ...
        x = np.random.normal(loc=x, scale=1, size=(self.num_particles,1))
        y = np.random.normal(loc=y, scale=1, size=(self.num_particles,1))
        theta = np.random.normal(loc=angle, scale=1, size=(self.num_particles,1))
